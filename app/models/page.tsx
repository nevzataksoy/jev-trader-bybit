...
const text = {
  tr: {
    ...
    intro: "Model1 mevcut karar hattını, Model2 ise rejim ve varlık rotasyonu yaklaşımını kullanır. İki motor aynı veri snapshot'ını görür; Jev kararları, sanal portföyleri ve emir kayıtları birbirinden ayrıdır.",
  ...
  en: {
    ...
    intro: "Model1 preserves the existing decision pipeline while Model2 uses a regime and asset-rotation approach. Both receive the same market snapshot; their Jev decisions, paper portfolios and order ledgers remain isolated.",
...
const emptyState: ModelsDashboardState = { generatedAt: new Date(0).toISOString(), runMode: "model1", exchangeRoutingForcedOff: false, database: "not_configured", message: null, experiment: null, engines: [], equity: [], orders: [], runs: [] };
...
function comparisonPath(points: EngineComparisonPoint[], engineId: "model1" | "model2", width: number, height: number) {
...
export default function ModelsPage() {
  ...
  return <main className="app-shell models-shell">
    ...
    <footer><Link className="brand brand--small" href="/"><span className="brand-mark"><i /><i /><i /></span><span><strong>JEV</strong> PULSE</span></Link><p>MODEL1 × MODEL2 • FORWARD PAPER OBSERVATION</p><span>{dateTime(data.generatedAt, lang)}</span></footer>
  </main>;
}
