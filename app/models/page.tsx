...
const text = {
  tr: {
    ...
    decisions: "Son karar ayrışmaları", decisionSub: "Her motorun aynı çevrimde verdiği aksiyonlar ve kararı oluşturan temel kanıtlar", orderHistory: "Paper emir geçmişi", orderSub: "Borsaya iletilmeyen fakat sanal bakiyeye işlenen emirler", time: "Zaman", engine: "Motor", asset: "Varlık", action: "Aksiyon", targetWeight: "Hedef ağırlık", confidence: "Güven", side: "Yön", quantity: "Miktar", fill: "Sanal fiyat", value: "Tutar", routing: "İletim", all: "Tüm motorlar", empty: "Henüz kayıt yok.", loadError: "A/B raporu alınamadı.",
    signal: "Sinyal", setupReadiness: "Kurulum / hazırlık", allocation: "Tahsis", edgeRisk: "Avantaj / risk", evidence: "Kanıt özeti", blockers: "Engelleyen filtreler", rationale: "Politika gerekçesi", details: "Ayrıntılar", runtime: "Runtime", executionEngine: "Exchange motoru", availableEngines: "Kayıtlı motorlar", cycle: "Çevrim", completed: "Tamamlandı", model: "Jev modeli", current: "Mevcut", targetLabel: "Hedef", delta: "Fark", expectedEdge: "Net avantaj", opportunity: "Fırsat", riskBudget: "Risk bütçesi", readinessScore: "Hazırlık puanı", signalState: "Sinyal durumu", regime: "Rejim", direction: "Yön", quality: "Kalite", liquidity: "Likidite", disorder: "Düzensizlik", falseBreakout: "Sahte kırılım", rowsPerPage: "Sayfa başına", previous: "Önceki", next: "Sonraki", page: "Sayfa", records: "kayıt",
    revision: "Politika revizyonu", structural: "Yapısal ekonomi", targetRoom: "Hedef alanı", invalidation: "Geçersizleşme", roundTripCost: "Tur maliyeti", atrCost: "ATR / maliyet", targetCost: "Hedef / maliyet", support: "Destek", resistance: "Direnç", executionAttempts: "Execution denemeleri", executionAttemptsSub: "Modelin BUY/SELL ürettiği noktada platform sonucunu ve tanısal oranları gösterir.", status: "Durum", reason: "Gerekçe", counterfactuals: "Reddedilen işlem karşı-olguları", counterfactualsSub: "Yalnız platformun reddettiği işlemler; mevcut 15 dakikalık cleanup +15m/+1h/+4h/+12h sonuçlarını otomatik doldurur.", forward: "İleri getiri",
  },
  en: {
    ...
    decisions: "Recent decision divergence", decisionSub: "Actions from each engine and the core evidence behind every decision", orderHistory: "Paper order history", orderSub: "Orders applied to paper balances but never routed to the exchange", time: "Time", engine: "Engine", asset: "Asset", action: "Action", targetWeight: "Target weight", confidence: "Confidence", side: "Side", quantity: "Quantity", fill: "Paper fill", value: "Value", routing: "Routing", all: "All engines", empty: "No records yet.", loadError: "The A/B report could not be loaded.",
    signal: "Signal", setupReadiness: "Setup / readiness", allocation: "Allocation", edgeRisk: "Edge / risk", evidence: "Evidence summary", blockers: "Blocking filters", rationale: "Policy rationale", details: "Details", runtime: "Runtime", executionEngine: "Exchange engine", availableEngines: "Registered engines", cycle: "Cycle", completed: "Completed", model: "Jev model", current: "Current", targetLabel: "Target", delta: "Delta", expectedEdge: "Net edge", opportunity: "Opportunity", riskBudget: "Risk budget", readinessScore: "Readiness score", signalState: "Signal state", regime: "Regime", direction: "Direction", quality: "Quality", liquidity: "Liquidity", disorder: "Disorder", falseBreakout: "False breakout", rowsPerPage: "Rows per page", previous: "Previous", next: "Next", page: "Page", records: "records",
    revision: "Policy revision", structural: "Structural economics", targetRoom: "Target room", invalidation: "Invalidation", roundTripCost: "Round-trip cost", atrCost: "ATR / cost", targetCost: "Target / cost", support: "Support", resistance: "Resistance", executionAttempts: "Execution attempts", executionAttemptsSub: "Shows platform outcomes and diagnostics whenever the model emits BUY/SELL.", status: "Status", reason: "Reason", counterfactuals: "Rejected-trade counterfactuals", counterfactualsSub: "Platform-rejected trades only; existing 15-minute cleanup fills +15m/+1h/+4h/+12h outcomes automatically.", forward: "Forward return",
  },
} as const;

const emptyState: ModelsDashboardState = { generatedAt: new Date(0).toISOString(), runMode: "loading", activeEngines: [], availableEngines: [], exchangeExecutionEngine: "none", exchangeRoutingForcedOff: false, database: "not_configured", message: null, experiment: null, engines: [], equity: [], orders: [], runs: [], executionAttempts: [], counterfactuals: [] };
...
const decisionValueLabels: Record<Language, Record<string, string>> = {
  ...
  en: {
    ...
    pending: "Pending", confirmed: "Confirmed", expired: "Expired", invalidated: "Invalidated", JEV_NO_ENTRY: "Jev: no entry", PENDING_CLOSE: "Waiting for close", PENDING_RETEST: "Waiting for retest", STRUCTURE_REJECTED: "Structure rejected", DIRECTIONAL_EDGE_LOW: "Directional edge low", SETUP_QUALITY_LOW: "Setup quality low", NET_EDGE_LOW: "Net edge low", LIQUIDITY_LOW: "Liquidity low", DISORDERLY_MARKET: "Disorderly market", RISK_BUDGET_ZERO: "Risk budget zero", ALLOCATION_DEADBAND: "Allocation delta below threshold",
  },
};
...
export default function ModelsPage() {
  ...
  const filteredAttempts = useMemo(() => data.executionAttempts.filter((attempt) => engineFilter === "all" || attempt.engineId === engineFilter), [data.executionAttempts, engineFilter]);
  const filteredCounterfactuals = useMemo(() => data.counterfactuals.filter((item) => engineFilter === "all" || item.engineId === engineFilter), [data.counterfactuals, engineFilter]);
  ...
  return <main className="app-shell models-shell">
    ...
    {!experiment ? <section className="panel section-panel empty-state">{t.noExperiment}</section> : <>
        ...
        <div className="table-scroll decisions-scroll">
          <table className="decisions-table">
            ...
            <tbody>{paginatedDecisionRows.length ? paginatedDecisionRows.map(({ run, decision }) => <tr key={`${run.engineId}-${run.cycleKey}-${decision.asset}`}>
              ...
              <td className="decision-engine" data-label={t.engine}><b>{run.engineId.toUpperCase()}</b><small>{run.engineVersion}</small><small>{t.model}: {run.jevModel ?? "—"}{run.latencyMs === null ? "" : ` · ${run.latencyMs} ms`}</small></td>
              ...
              <td className="decision-edge" data-label={t.edgeRisk}><span><small>{t.expectedEdge}</small><b className={decision.expectedNetEdgePct > 0 ? "positive" : decision.expectedNetEdgePct < 0 ? "negative" : ""}>{decision.expectedNetEdgePct > 0 ? "+" : ""}{percentage(decision.expectedNetEdgePct, 3)}</b></span><span><small>{t.opportunity}</small><b>{decision.opportunityScore.toFixed(2)}</b></span><span><small>{t.riskBudget}</small><b>{percentage(decision.grossRiskBudgetPct, 2)}</b></span></td>
              <td className="decision-details" data-label={t.details}>
                ...
                {decision.diagnostics && <DetailBadge label={t.structural} tone="info"><div className="decision-evidence"><span><small>{t.targetRoom}</small><b>{percentage(decision.diagnostics.targetDistancePct, 3)}</b></span><span><small>{t.invalidation}</small><b>{percentage(decision.diagnostics.invalidationDistancePct, 3)}</b></span><span><small>{t.roundTripCost}</small><b>{percentage(decision.diagnostics.roundTripCostPct, 3)}</b></span><span><small>{t.atrCost}</small><b>{decision.diagnostics.atrToCostRatio.toFixed(2)}×</b></span><span><small>{t.targetCost}</small><b>{decision.diagnostics.targetToCostRatio.toFixed(2)}×</b></span><span><small>{t.support}</small><b>{money(decision.diagnostics.supportZoneLow, lang)}–{money(decision.diagnostics.supportZoneHigh, lang)} · {probability(decision.diagnostics.supportStrength)}</b></span><span><small>{t.resistance}</small><b>{money(decision.diagnostics.resistanceZoneLow, lang)}–{money(decision.diagnostics.resistanceZoneHigh, lang)} · {probability(decision.diagnostics.resistanceStrength)}</b></span></div></DetailBadge>}
      ...
      <section className="panel section-panel orders-panel">
        <div className="panel-heading"><div><span className="section-kicker">EXECUTION</span><h2>{t.executionAttempts}</h2><p>{t.executionAttemptsSub}</p></div></div>
        <div className="table-scroll"><table><thead><tr><th>{t.time}</th><th>{t.engine}</th><th>{t.asset}</th><th>{t.action}</th><th>{t.status}</th><th>{t.atrCost}</th><th>{t.targetCost}</th><th>{t.reason}</th></tr></thead><tbody>
          {filteredAttempts.length ? filteredAttempts.slice(0, 100).map((attempt, index) => <tr key={`${attempt.engineId}-${attempt.cycleKey}-${attempt.asset}-${attempt.action}-${index}`}><td>{dateTime(attempt.createdAt, lang)}</td><td><b>{attempt.engineId.toUpperCase()}</b><small className="order-id">{attempt.policyRevision ?? attempt.engineVersion}</small></td><td>{attempt.asset}</td><td><span className={`side-badge side-badge--${attempt.action}`}>{decisionValue(attempt.action, lang).toUpperCase()}</span></td><td><span className="order-status">{attempt.status}</span></td><td>{attempt.diagnostics?.atrToCostRatio === undefined ? "—" : `${attempt.diagnostics.atrToCostRatio.toFixed(2)}×`}</td><td>{attempt.diagnostics?.targetToCostRatio === undefined ? "—" : `${attempt.diagnostics.targetToCostRatio.toFixed(2)}×`}</td><td>{attempt.reason}</td></tr>) : <tr><td className="empty-cell" colSpan={8}>{t.empty}</td></tr>}
        </tbody></table></div>
      </section>
      <section className="panel section-panel orders-panel">
        <div className="panel-heading"><div><span className="section-kicker">COUNTERFACTUAL</span><h2>{t.counterfactuals}</h2><p>{t.counterfactualsSub}</p></div></div>
        <div className="table-scroll"><table><thead><tr><th>{t.time}</th><th>{t.engine}</th><th>{t.asset}</th><th>{t.setupReadiness}</th><th>{t.expectedEdge}</th><th>+15m</th><th>+1h</th><th>+4h</th><th>+12h</th><th>{t.reason}</th></tr></thead><tbody>
          {filteredCounterfactuals.length ? filteredCounterfactuals.slice(0, 100).map((item) => <tr key={item.id}><td>{dateTime(item.createdAt, lang)}</td><td><b>{item.engineId.toUpperCase()}</b><small className="order-id">{item.policyRevision ?? item.engineVersion}</small></td><td>{item.asset}</td><td><b>{decisionValue(item.setup, lang)}</b><small className="order-id">{decisionValue(item.readiness, lang)}</small></td><td>{item.expectedNetEdgePct === null ? "—" : percentage(item.expectedNetEdgePct, 3)}</td><td>{item.forward15mPct === null ? "—" : percentage(item.forward15mPct, 3)}</td><td>{item.forward1hPct === null ? "—" : percentage(item.forward1hPct, 3)}</td><td>{item.forward4hPct === null ? "—" : percentage(item.forward4hPct, 3)}</td><td>{item.forward12hPct === null ? "—" : percentage(item.forward12hPct, 3)}</td><td>{item.rejectionReason}</td></tr>) : <tr><td className="empty-cell" colSpan={10}>{t.empty}</td></tr>}
        </tbody></table></div>
      </section>
