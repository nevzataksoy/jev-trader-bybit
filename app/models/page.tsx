...
const text = {
  tr: {
    ...
    signal: "Sinyal", revision: "Revizyon", status: "Durum", reason: "Neden", metrics: "Metrikler", setupReadiness: "Kurulum / hazırlık", allocation: "Tahsis", edgeRisk: "Avantaj / risk", evidence: "Kanıt özeti", blockers: "Engelleyen filtreler", rationale: "Politika gerekçesi", details: "Ayrıntılar", runtime: "Runtime", executionEngine: "Exchange motoru", availableEngines: "Kayıtlı motorlar", cycle: "Çevrim", completed: "Tamamlandı", model: "Jev modeli", current: "Mevcut", targetLabel: "Hedef", delta: "Fark", expectedEdge: "Net avantaj", opportunity: "Fırsat", riskBudget: "Risk bütçesi", readinessScore: "Hazırlık puanı", signalState: "Sinyal durumu", regime: "Rejim", direction: "Yön", quality: "Kalite", liquidity: "Likidite", disorder: "Düzensizlik", falseBreakout: "Sahte kırılım", rowsPerPage: "Sayfa başına", previous: "Önceki", next: "Sonraki", page: "Sayfa", records: "kayıt",
  },
  en: {
...
const decisionValueLabels: Record<Language, Record<string, string>> = {
  ...
  en: {
    ...
    pending: "Pending", confirmed: "Confirmed", expired: "Expired", invalidated: "Invalidated", JEV_NO_ENTRY: "Jev: no entry", PENDING_CLOSE: "Waiting for close", PENDING_RETEST: "Waiting for retest", STRUCTURE_REJECTED: "Structure rejected", DIRECTIONAL_EDGE_LOW: "Directional edge low", SETUP_QUALITY_LOW: "Setup quality low", NET_EDGE_LOW: "Net edge low", LIQUIDITY_LOW: "Liquidity low", DISORDERLY_MARKET: "Disorderly market", RISK_BUDGET_ZERO: "Risk budget zero", ALLOCATION_DEADBAND: "Allocation delta below threshold", NO_ALLOCATION_INTENT: "No allocation intent", TARGET_ROOM_LOW: "Target room too small after costs",
  },
};
        ...
        <div className="table-scroll decisions-scroll">
          <table className="decisions-table">
            ...
            <tbody>{paginatedDecisionRows.length ? paginatedDecisionRows.map(({ run, decision }) => <tr key={`${run.engineId}-${run.cycleKey}-${decision.asset}`}>
              ...
              <td className="decision-edge" data-label={t.edgeRisk}><span><small>{t.expectedEdge}</small><b className={decision.expectedNetEdgePct > 0 ? "positive" : decision.expectedNetEdgePct < 0 ? "negative" : ""}>{decision.expectedNetEdgePct > 0 ? "+" : ""}{percentage(decision.expectedNetEdgePct, 3)}</b></span><span><small>GROSS / COST</small><b>{percentage(decision.grossExpectedEdgePct ?? 0, 3)} / {percentage(decision.roundTripCostPct ?? 0, 3)}</b></span><span><small>TARGET / INVALID.</small><b>{percentage(decision.targetDistancePct ?? 0, 3)} / {percentage(decision.invalidationDistancePct ?? 0, 3)}</b></span><span><small>R:R</small><b>{(decision.rewardRiskRatio ?? 0).toFixed(2)}</b></span><span><small>{t.opportunity}</small><b>{decision.opportunityScore.toFixed(2)}</b></span><span><small>{t.riskBudget}</small><b>{percentage(decision.grossRiskBudgetPct, 2)}</b></span></td>
