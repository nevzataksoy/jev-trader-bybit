...
const text = {
  tr: {
    ...
    signal: "Sinyal", setupReadiness: "Kurulum / hazırlık", allocation: "Tahsis", edgeRisk: "Avantaj / risk", evidence: "Kanıt özeti", blockers: "Engelleyen filtreler", rationale: "Politika gerekçesi", cycle: "Çevrim", completed: "Tamamlandı", model: "Jev modeli", current: "Mevcut", targetLabel: "Hedef", delta: "Fark", expectedEdge: "Net avantaj", opportunity: "Fırsat", riskBudget: "Risk bütçesi", readinessScore: "Hazırlık puanı", signalState: "Sinyal durumu", regime: "Rejim", direction: "Yön", quality: "Kalite", liquidity: "Likidite", disorder: "Düzensizlik", falseBreakout: "Sahte kırılım", rowsPerPage: "Sayfa başına", previous: "Önceki", next: "Sonraki", page: "Sayfa", records: "kayıt",
  },
  en: {
      ...
      <section className="panel section-panel orders-panel decisions-panel">
        ...
        <div className="table-scroll">
          <table className="decisions-table">
            <thead><tr><th>{t.time}</th><th>{t.engine}</th><th>{t.signal}</th><th>{t.setupReadiness}</th><th>{t.allocation}</th><th>{t.edgeRisk}</th><th>{t.evidence}</th><th>{t.blockers}</th><th>{t.rationale}</th></tr></thead>
            <tbody>{paginatedDecisionRows.length ? paginatedDecisionRows.map(({ run, decision }) => <tr key={`${run.engineId}-${run.cycleKey}-${decision.asset}`}>
              <td className="decision-time"><strong>{dateTime(run.cycleKey, lang)}</strong><small>{t.completed}: {dateTime(run.completedAt, lang)}</small></td>
              <td className="decision-engine"><b>{run.engineId.toUpperCase()}</b><small>{run.engineVersion}</small><small>{t.model}: {run.jevModel ?? "—"}{run.latencyMs === null ? "" : ` · ${run.latencyMs} ms`}</small></td>
              <td className="decision-signal"><div><strong>{decision.asset}/USDT</strong><span className={`side-badge side-badge--${decision.action}`}>{decisionValue(decision.action, lang).toUpperCase()}</span></div><small>{t.confidence}: <b>{probability(decision.confidence)}</b></small><div className="decision-probabilities"><span className="positive">{decisionValue("buy", lang).toUpperCase()} {probability(decision.probabilities.buy)}</span><span>{decisionValue("hold", lang).toUpperCase()} {probability(decision.probabilities.hold)}</span><span className="negative">{decisionValue("sell", lang).toUpperCase()} {probability(decision.probabilities.sell)}</span></div></td>
              <td className="decision-setup"><strong>{decisionValue(decision.selectedSetup, lang)}</strong><span>{decisionValue(decision.entryReadiness, lang)}</span><small>{t.quality}: {decision.judgments.setup_quality.score.toFixed(2)}/4 · {t.confidence} {probability(decision.judgments.setup_quality.confidence)}</small>{decision.readinessScore !== undefined && <small>{t.readinessScore}: {probability(decision.readinessScore)}</small>}{decision.signalState && <small>{t.signalState}: {decisionValue(decision.signalState, lang)}</small>}</td>
              <td className="decision-allocation"><span><small>{t.current}</small><b>{percentage(decision.currentAllocationPct, 2)}</b></span><i>→</i><span><small>{t.targetLabel}</small><b>{percentage(decision.targetAllocationPct, 2)}</b></span><em className={decision.rebalanceDeltaPct > 0 ? "positive" : decision.rebalanceDeltaPct < 0 ? "negative" : ""}>{t.delta}: {decision.rebalanceDeltaPct > 0 ? "+" : ""}{percentage(decision.rebalanceDeltaPct, 2)}</em></td>
              <td className="decision-edge"><span><small>{t.expectedEdge}</small><b className={decision.expectedNetEdgePct > 0 ? "positive" : decision.expectedNetEdgePct < 0 ? "negative" : ""}>{decision.expectedNetEdgePct > 0 ? "+" : ""}{percentage(decision.expectedNetEdgePct, 3)}</b></span><span><small>{t.opportunity}</small><b>{decision.opportunityScore.toFixed(2)}</b></span><span><small>{t.riskBudget}</small><b>{percentage(decision.grossRiskBudgetPct, 2)}</b></span></td>
              <td className="decision-evidence"><span><small>{t.regime}</small><b>{decisionValue(decision.judgments.regime.choice, lang)} · {probability(decision.judgments.regime.confidence)}</b></span><span><small>{t.direction}</small><b>{decisionValue(decision.judgments.direction.choice, lang)} · {probability(decision.judgments.direction.confidence)}</b></span><span><small>{t.liquidity}</small><b>{probability(decision.judgments.liquidity_ok)}</b></span><span><small>{t.disorder}</small><b>{probability(decision.judgments.disorderly)}</b></span><span><small>{t.falseBreakout}</small><b>{probability(decision.judgments.false_breakout)}</b></span></td>
              <td className="decision-blockers">{decision.blockedBy?.length ? decision.blockedBy.map((blocker) => <span key={blocker}>{decisionValue(blocker, lang)}</span>) : <span className="decision-clear">CLEAR</span>}</td>
              <td className="decision-rationale">{decision.policyReason}</td>
            </tr>) : <tr><td className="empty-cell" colSpan={9}>{t.empty}</td></tr>}</tbody>
          </table>
        </div>
