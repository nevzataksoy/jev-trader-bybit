...
const text = {
  tr: {
    ...
    signal: "Sinyal", revision: "Revizyon", status: "Durum", reason: "Neden", metrics: "Metrikler", setupReadiness: "Kurulum / hazırlık", allocation: "Tahsis", edgeRisk: "Avantaj / risk", evidence: "Kanıt özeti", blockers: "Engelleyen filtreler", diagnostics: "Tanısal sinyaller", rationale: "Politika gerekçesi", details: "Ayrıntılar", runtime: "Runtime", executionEngine: "Exchange motoru", availableEngines: "Kayıtlı motorlar", cycle: "Çevrim", completed: "Tamamlandı", model: "Jev modeli", current: "Mevcut", targetLabel: "Hedef", delta: "Fark", expectedEdge: "Net avantaj", opportunity: "Fırsat", candidatePlan: "Aday plan", afterCostRoom: "Maliyet sonrası alan", shadowForecast: "Gölge olasılık", targetFirst: "T1 önce", invalidationFirst: "Invalidation önce", timeout: "Zaman aşımı", shadowExpected: "Gölge beklenen net", riskBudget: "Portföy risk bütçesi", readinessScore: "Hazırlık puanı", signalState: "Sinyal durumu", regime: "Rejim", direction: "Yön", quality: "Kalite", liquidity: "Likidite", disorder: "Düzensizlik", falseBreakout: "Sahte kırılım", rowsPerPage: "Sayfa başına", previous: "Önceki", next: "Sonraki", page: "Sayfa", records: "kayıt",
  },
  en: {
...
const decisionValueLabels: Record<Language, Record<string, string>> = {
  ...
  en: {
    ...
    pending: "Pending", confirmed: "Confirmed", expired: "Expired", invalidated: "Invalidated", JEV_NO_ENTRY: "Jev: no entry", PENDING_CLOSE: "Waiting for close", PENDING_RETEST: "Waiting for retest", STRUCTURE_REJECTED: "Structure rejected", DIRECTIONAL_EDGE_LOW: "Directional edge low", SETUP_QUALITY_LOW: "Setup quality low", NET_EDGE_LOW: "Net edge low", LIQUIDITY_LOW: "Liquidity low", DISORDERLY_MARKET: "Disorderly market", THESIS_INVALIDATED: "Long thesis invalidated", RISK_BUDGET_ZERO: "Risk budget zero", ALLOCATION_DEADBAND: "Allocation delta below threshold", NO_ALLOCATION_INTENT: "No allocation intent", TARGET_ROOM_LOW: "Target room too small after costs", inside_support: "Inside support", near_support: "Near support", between_levels: "Between levels", inside_resistance: "Inside resistance", unstructured: "Unstructured", resistance: "Resistance", atr_projection: "ATR projection",
  },
};
        ...
        <div className="table-scroll decisions-scroll">
          <table className="decisions-table">
            ...
            <tbody>{paginatedDecisionRows.length ? paginatedDecisionRows.map(({ run, decision }) => <tr key={`${run.engineId}-${run.cycleKey}-${decision.asset}`}>
              ...
              <td className="decision-edge" data-label={t.edgeRisk}><span><small>{t.expectedEdge}</small><b className={decision.expectedNetEdgePct > 0 ? "positive" : decision.expectedNetEdgePct < 0 ? "negative" : ""}>{decision.expectedNetEdgePct > 0 ? "+" : ""}{percentage(decision.expectedNetEdgePct, 3)}</b></span><span><small>GROSS / COST</small><b>{percentage(decision.grossExpectedEdgePct ?? 0, 3)} / {percentage(decision.roundTripCostPct ?? 0, 3)}</b></span><span><small>TARGET / REWARD / INVALID.</small><b>{percentage(decision.targetDistancePct ?? 0, 3)} / {percentage(decision.rewardDistancePct ?? decision.targetDistancePct ?? 0, 3)} / {percentage(decision.invalidationDistancePct ?? 0, 3)}</b><small>{decision.rewardSource ? decisionValue(decision.rewardSource, lang) : "legacy"}</small></span><span><small>R:R</small><b>{(decision.rewardRiskRatio ?? 0).toFixed(2)}</b></span>{decision.candidatePlan && <span><small>{t.candidatePlan}</small><b>{decisionValue(decision.candidatePlan.location, lang)}</b><small>{t.afterCostRoom}: {percentage(decision.candidatePlan.target1AfterCostRoomPct, 3)}</small></span>}{decision.shadowForecast && <span><small>{t.shadowForecast}</small><b>{t.targetFirst} {percentage(decision.shadowForecast.target1BeforeInvalidation * 100, 1)} · {t.invalidationFirst} {percentage(decision.shadowForecast.invalidationBeforeTarget1 * 100, 1)} · {t.timeout} {percentage(decision.shadowForecast.timeout * 100, 1)}</b><small>{t.shadowExpected}: {percentage(decision.shadowForecast.expectedNetReturnPct, 3)} · {decision.shadowForecast.status}</small></span>}<span><small>{t.opportunity}</small><b>{decision.opportunityScore.toFixed(2)}</b></span><span><small>{t.riskBudget}</small><b>{percentage(decision.grossRiskBudgetPct, 2)}</b></span></td>
