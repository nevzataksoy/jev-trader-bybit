...
const text = {
  tr: {
    ...
    signal: "Sinyal", setupReadiness: "Kurulum / hazırlık", allocation: "Tahsis", edgeRisk: "Avantaj / risk", evidence: "Kanıt özeti", rationale: "Politika gerekçesi", cycle: "Çevrim", completed: "Tamamlandı", model: "Jev modeli", current: "Mevcut", targetLabel: "Hedef", delta: "Fark", expectedEdge: "Net avantaj", opportunity: "Fırsat", riskBudget: "Risk bütçesi", regime: "Rejim", direction: "Yön", quality: "Kalite", liquidity: "Likidite", disorder: "Düzensizlik", falseBreakout: "Sahte kırılım", rowsPerPage: "Sayfa başına", previous: "Önceki", next: "Sonraki", page: "Sayfa", records: "kayıt",
  },
  en: {
    ...
    signal: "Signal", setupReadiness: "Setup / readiness", allocation: "Allocation", edgeRisk: "Edge / risk", evidence: "Evidence summary", rationale: "Policy rationale", cycle: "Cycle", completed: "Completed", model: "Jev model", current: "Current", targetLabel: "Target", delta: "Delta", expectedEdge: "Net edge", opportunity: "Opportunity", riskBudget: "Risk budget", regime: "Regime", direction: "Direction", quality: "Quality", liquidity: "Liquidity", disorder: "Disorder", falseBreakout: "False breakout", rowsPerPage: "Rows per page", previous: "Previous", next: "Next", page: "Page", records: "records",
  },
} as const;
      ...
      <section className="panel section-panel orders-panel decisions-panel">
        ...
        <div className="table-scroll">
          <table className="decisions-table">
            <thead><tr><th>{t.time}</th><th>{t.engine}</th><th>{t.signal}</th><th>{t.setupReadiness}</th><th>{t.allocation}</th><th>{t.edgeRisk}</th><th>{t.evidence}</th><th>{t.rationale}</th></tr></thead>
            <tbody>{paginatedDecisionRows.length ? paginatedDecisionRows.map(({ run, decision }) => <tr key={`${run.engineId}-${run.cycleKey}-${decision.asset}`}>
              ...
              <td className="decision-setup"><strong>{decisionValue(decision.selectedSetup, lang)}</strong><span>{decisionValue(decision.entryReadiness, lang)}</span><small>{t.quality}: {decision.judgments.setup_quality.score.toFixed(2)}/4 · {t.confidence} {probability(decision.judgments.setup_quality.confidence)}</small></td>
            ...
            </tr>) : <tr><td className="empty-cell" colSpan={8}>{t.empty}</td></tr>}</tbody>
          </table>
        </div>
