...
export function buildBlindModel1State(state: Parameters<StrategyEngine["evaluate"]>[0]) {
...
function regimeQuestion(slot: BlindSlot) {
  return choice(
    { objective: `Classify the executable regime in state.candidates.${slot}.`, constraints: ["Use only supplied evidence.", "Choose transition when material evidence conflicts.", "Do not infer instrument or calendar identity."] },
    { uptrend: "Persistent higher structure.", downtrend: "Persistent lower structure.", range: "Stable mean-reverting boundaries.", compression: "Compressed volatility awaiting confirmation.", transition: "Changing or conflicting structure." },
  );
}

function setupQuestion(slot: BlindSlot) {
  return choice(
    { objective: `Select the best long-only setup supported by state.candidates.${slot}.`, constraints: ["Select none without positive executable evidence.", "A range entry belongs near a lower boundary.", "A breakout needs participation.", "Do not output an order."] },
    { trend_pullback: "Orderly pullback in an intact rise.", upside_breakout: "Accepted upper-boundary break.", range_reversion: "Supported lower-boundary mean reversion.", bear_rebound: "Confirmed tactical rebound in a decline.", reduce: "Existing long thesis is deteriorating.", none: "No executable setup." },
  );
}

function readinessQuestion(slot: BlindSlot) {
  return choice(
    { objective: `Judge entry readiness for state.candidates.${slot}.`, constraints: ["Closed evidence must confirm enter_now.", "Do not infer identity."] },
    { enter_now: "Confirmed now.", wait_close: "Needs another close.", wait_retest: "Needs a retest.", no_entry: "No acceptable entry." },
  );
}

function directionQuestion(slot: BlindSlot) {
  return choice(
    { objective: `Judge executable direction for state.candidates.${slot} over the next one to four decision cycles.`, constraints: ["Use unclear when evidence conflicts."] },
    { up: "Higher after costs is more likely.", down: "Lower is more likely.", unclear: "No separated direction." },
  );
}

function followThroughQuestion(slot: BlindSlot) {
  return choice(
    { objective: `Judge persistence of the visible move in state.candidates.${slot}.`, constraints: ["Separate persistence from direction."] },
    { continuation: "Structure supports persistence.", reversal: "Exhaustion or contradiction supports reversal.", no_pattern: "No stable pattern." },
  );
}

function setupQualityQuestion(slot: BlindSlot) {
  return score(
    { objective: `Score setup quality for state.candidates.${slot}.`, constraints: ["Score evidence agreement and tradability, not excitement."] },
    ["No usable setup.", "Weak setup.", "Developing setup.", "Coherent setup.", "Exceptional setup."],
  );
}
...
export const model1BlindEngine: StrategyEngine = {
  ...
  async evaluate(engineState) {
    ...
    const portfolioJudgments = buildDeterministicPortfolioJudgments(judgments);
    ...
    return {
      ...
      decisions: buildPortfolioDecisions(judgments, portfolioJudgments, engineState.indicators, engineState.positions, engineState.macro, feePctByAsset, getTradingConfig()),
      portfolioJudgments,
