...
export function planExecution(
  ...
  if (decision.action === "hold") return deny("Model1 V1 selected hold for this cycle.");
  ...
  if (decision.action === "sell") {
    ...
    return {
      allowed: true,
      reason: `Model1 V1 risk-reduction sizing requests ${(sellPctOfHolding * 100).toFixed(1)}% of the holding.`,
      buyPctOfUsdt: 0,
      sellPctOfHolding,
    };
  }
  ...
  if (context.market.regime === "bear_trend" && context.market.countertrend_rebound_score < config.minBearReboundScore) {
    return deny(`Model1 V1 bear-market rebound score ${context.market.countertrend_rebound_score.toFixed(2)} is below ${config.minBearReboundScore.toFixed(2)}.`);
  }
  ...
  const requestedSpend = isInitialEntry
    ? context.totalPortfolioUsdt * config.initialEntryPctOfPortfolio
    : freeUsdt * config.buyPctOfUsdt;
  const desiredSpend = Math.min(requestedSpend, targetDeltaSpend) * volatilityScale;
  return {
    allowed: true,
    reason: `Model1 V1 sizing requests ${desiredSpend.toFixed(2)} USDT after ${(volatilityScale * 100).toFixed(0)}% volatility scaling.`,
