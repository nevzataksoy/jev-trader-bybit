...
export async function applyConfirmation(input: {
  ...
  for (const original of input.decisions) {
    ...
    if (signal && (hasHardFatalBlocker(decision) || decision.selectedSetup !== signal.setup)) {
      await resolveSignal(signal.id, "invalidated", input.cycleKey, "Fresh evidence invalidated or replaced the rotation setup.");
      signal = undefined;
    ...
    if (signal) {
      ...
      if (confirmation.confirmed) {
        ...
        decision = {
          ...
          policyReason: `${decision.policyReason} Model2 V2 is awaiting a later closed-candle confirmation for the active ${signal.readiness} rotation watch.`,
        };
      }
    }

    const readiness = hasPendingBlocker(decision);
    if (!signal && decision.signalState !== "confirmed" && readiness && !hasHardFatalBlocker(decision)) {
