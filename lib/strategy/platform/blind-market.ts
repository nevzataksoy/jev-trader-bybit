...
export function buildBlindNumericState(state: JevTradingState) {
  ...
  const candidates = Object.fromEntries(BLIND_SLOTS.map((slot) => {
    ...
    return [slot, {
      ...
      risk_and_participation: {
        atr_14_pct: round(market.atr_14_pct), atr_14_1h_pct: round(market.atr_14_1h_pct ?? market.atr_14_pct * 2),
        atr_14_4h_pct: round(market.atr_14_4h_pct ?? market.atr_14_pct * 4), atr_15m_percentile: round(market.atr_15m_percentile ?? 0.5),
      ...
      execution: {
        ...
        orderbook_imbalance: round(market.orderbook_imbalance), orderbook_wall_bias: round(market.orderbook_wall_bias ?? 0),
        bid_wall_distance_pct: round(market.largest_bid_wall_distance_pct ?? 0), bid_wall_share: round(market.bid_wall_share ?? 0),
        bid_wall_persistence: round(market.bid_wall_persistence ?? 0.25), ask_wall_distance_pct: round(market.largest_ask_wall_distance_pct ?? 0),
        ask_wall_share: round(market.ask_wall_share ?? 0), ask_wall_persistence: round(market.ask_wall_persistence ?? 0.25),
